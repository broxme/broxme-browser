...
export class ExplorerApp extends LitElement {
  ...
  constructor () {
    ...
    broxme.panes.addEventListener('pane-attached', e => {
      this.attachedPane = broxme.panes.getAttachedPane()
      if (loc.getUrl() !== this.attachedPane.url) {
        loc.setUrl(this.attachedPane.url)
      }
    })
    broxme.panes.addEventListener('pane-detached', e => {
      this.attachedPane = undefined
    })
    broxme.panes.addEventListener('pane-navigated', e => {
    ...
    ;(async () => {
      ...
      if (this.attachedPane) {
        ...
        if (loc.getUrl() && loc.getUrl() !== this.attachedPane.url) {
          broxme.panes.navigate(this.attachedPane.id, loc.getUrl())
        }
      } else {
        // try to attach and then follow their url
        this.attachedPane = await broxme.panes.attachToLastActivePane()
  ...
  async load () {
    ...
    var drive = broxme.hyperdrive.drive(loc.getOrigin())
    try {
    ...
    if (this.pathInfo.isDirectory()) {
      ...
      if (!this.watchStream) {
        let currentDrive = broxme.hyperdrive.drive(this.currentDriveInfo.url)
    ...
    while (pathParts.length) {
      ...
      if (stat.mount) {
        mount = await this.attempt(
          ...
          () => broxme.hyperdrive.drive(stat.mount.key).getInfo()
        )
      }
    ...
    for (let item of items) {
      ...
      if (item.stat.mount) {
        item.mount = await this.attempt(
          ...
          () => broxme.hyperdrive.drive(item.stat.mount.key).getInfo()
        )
      }
  ...
  async readViewfile (drive) {
    ...
    await this.attempt(
      ...
      item.url = joinPath(loc.getOrigin(), item.path)
      item.shareUrl = this.getShareUrl(item)
      item.drive = await broxme.hyperdrive.drive(item.drive).getInfo()
      item.mount = item.mount ? broxme.hyperdrive.drive(item.mount).getInfo() : undefined
      this.setItemIcons(item)
    })))
  ...
  render () {
    return html`
      <link rel="stylesheet" href="broxme://explorer/css/font-awesome.css">
      <div
        class=${classMap({
          layout: true,
  ...
  goto (item, newWindow = false, leaveExplorer = false) {
    ...
    if (this.attachedPane) {
      ...
      else broxme.panes.navigate(this.attachedPane.id, url)
    } else {
      if (leaveExplorer) {
        if (newWindow) window.open(url)
  ...
  async onNewFile (e) {
    ...
    if (filename) {
      ...
      var drive = broxme.hyperdrive.drive(this.currentDriveInfo.url)
      ...
      window.open(`broxme://editor/?url=${url}`)
    }
  }
  ...
  async onNewMount (e) {
    if (!this.currentDriveInfo.writable) return
    var drive = broxme.hyperdrive.drive(this.currentDriveInfo.url)
    var targetUrl = await broxme.shell.selectDriveDialog({title: 'Select a site'})
    var target = broxme.hyperdrive.drive(targetUrl)
  ...
  async onForkDrive (e) {
    var drive = await broxme.hyperdrive.forkDrive(this.currentDriveInfo.url)
    toast.create('Site created')
    loc.setUrl(drive.url)
  }

  async onDriveProperties (e) {
    await broxme.shell.drivePropertiesDialog(this.currentDriveInfo.url)
    this.load()
  }
  ...
  async onImportFiles (e) {
    ...
    try {
      var {numImported} = await broxme.shell.importFilesDialog(loc.getUrl())
  ...
  async onImportFolders (e) {
    ...
    toast.create('Importing...')
    try {
      var {numImported} = await broxme.shell.importFoldersDialog(loc.getUrl())
      if (numImported > 0) toast.create('Import complete', 'success')
      else toast.destroy()
    } catch (e) {
      console.log(e)
  ...
  async onExport (e) {
    ...
    try {
      var {numExported} = await broxme.shell.exportFilesDialog(urls)
  ...
  async onShowMenu (e, useAppMenuAlways = false) {
    ...
    if (!useAppMenuAlways && typeof broxme !== 'undefined' && typeof broxme.browser !== 'undefined') {
      let fns = {}
      ...
      var choice = await broxme.browser.showContextMenu(items, true)
      ...
      return contextMenu.create({
        ...
        roomy: false,
        noBorders: true,
        fontAwesomeCSSUrl: 'broxme://explorer/css/font-awesome.css',
        style: `padding: 4px 0`,
  ...
  async doDiff (base) {
    if (this.attachedMode) broxme.browser.gotoUrl(`broxme://diff/?base=${base}`)
    else window.open(`broxme://diff/?base=${base}`)
  }

  async onSelectDrive (e) {
    e.preventDefault()
    var drive = await broxme.shell.selectDriveDialog()
    loc.setUrl(drive)
  }
}
