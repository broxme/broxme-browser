...
class DriveForks extends LitElement {
  ...
  async onClickNewFork (e) {
    e.preventDefault()
    e.stopPropagation()
    var drive = await broxme.hyperdrive.forkDrive(this.origin)
  ...
  onClickDiff (e, fork) {
    e.preventDefault()
    e.stopPropagation()
    broxme.browser.openUrl(`broxme://diff/?base=${fork.url}&target=${this.forks[0].url}`, {setActive: true})
  ...
  async onClickDelete (e, fork) {
    ...
    await broxme.drives.remove(fork.url)
