...
class SiteInfoApp extends LitElement {
  ...
  get isBeaker () {
    return this.url && this.url.startsWith('broxme:')
  }

  get isRootDrive () {
    return this.origin === broxme.hyperdrive.drive('hyper://private/').url
  }
  ...
  constructor () {
    ...
    window.addEventListener('blur', e => {
      broxme.browser.toggleSiteInfo(false)
      this.reset()
    })
    ...
    const globalAnchorClickHandler = (isPopup) => e => {
      ...
      if (a) {
        ...
        if (href && href !== '#' && !href.startsWith('broxme://')) {
          if (isPopup || e.metaKey || a.getAttribute('target') === '_blank') {
            broxme.browser.openUrl(href, {setActive: true})
          } else {
            broxme.browser.gotoUrl(href)
          }
          broxme.browser.toggleSiteInfo(false)
        }
      }
    ...
    try {
      ...
      if (this.isDrive) {
        ...
        ;[this.info, this.driveCfg, this.forks] = await Promise.all([
          drive.getInfo(),
          broxme.drives.get(this.url),
          broxme.drives.getForks(this.url)
        ])
      } else {
        this.info = {
          title: this.hostname,
      ...
      ;[perms, this.cert] = await Promise.all([
        broxme.sitedata.getPermissions(this.origin),
        broxme.browser.getCertificate(this.url)
      ])
      ...
      this.requestedPerms = await Promise.all(Object.entries(perms).map(async ([perm, value]) => {
        ...
        if (isHyperHashRegex.test(permParam)) {
          let driveInfo
          try { driveInfo = await broxme.broxme.hyperdrive.drive(permParam).getInfo() }
          catch (e) { /* ignore */ }
  ...
  async updated () {
    setTimeout(() => {
      ...
      if (!height) return
      broxme.browser.resizeSiteInfo({height})
    }, 50)
  }
  ...
  onChangeUrl (e) {
    ...
    broxme.browser.gotoUrl(this.url)
    this.load()
  }
  ...
  async onClickDriveProperties (e) {
    await broxme.shell.drivePropertiesDialog(this.url)
    this.load()
  }

  async onToggleSaveDrive (e) {
    if (this.driveCfg && this.driveCfg.saved) {
      await broxme.drives.remove(this.origin)
    } else {
      await broxme.drives.configure(this.origin)
    }
    this.load()
  }
    ...
    return contextMenu.create({
      ...
      items: [
        ...
        this.info && this.info.writable ? {
          ...
          click: async () => {
            await broxme.folderSync.syncDialog(this.info.url)
            await broxme.browser.refreshTabState()
          }
        } : undefined,
        {
  ...
  async onForkDrive () {
    var drive = await broxme.hyperdrive.forkDrive(this.url, {detached: false})
    broxme.browser.openUrl(drive.url, {setActive: true})
  }
