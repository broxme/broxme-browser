...
import { emit } from 'broxme://app-stdlib/js/dom.js'
import { pluralize, toNiceDomain } from 'broxme://app-stdlib/js/strings.js'
import { isFilenameBinary } from 'broxme://app-stdlib/js/is-ext-binary.js'
import * as toast from 'broxme://app-stdlib/js/com/toast.js'
import * as contextMenu from 'broxme://app-stdlib/js/com/context-menu.js'
import * as QP from 'broxme://app-stdlib/js/query-params.js'
...
export class CompareApp extends LitElement {
  ...
  async load () {
    if (!this.otherDrives) {
      ;(async () => {
        this.otherDrives = await broxme.drives.list({includeSystem: false})
    ...
    this.baseDrive = this.base ? broxme.hyperdrive.drive(this.base) : undefined
    this.targetDrive = this.target ? broxme.hyperdrive.drive(this.target) : undefined
    await this.attempt(
    ...
    this.baseForks = this.baseInfo
      ? await this.attempt(
        ...
        () => broxme.drives.getForks(this.baseInfo.url)
       ) : undefined
    this.targetForks = this.targetInfo
      ? await this.attempt(
        'Fetching known forks of the target drive',
        () => broxme.drives.getForks(this.targetInfo.url)
       ) : undefined
  ...
  render () {
    ...
    return html`
      <link rel="stylesheet" href="broxme://app-stdlib/css/fontawesome.css">
      <header>
        <div class="toolbar">
          <div class="title">
            Merging
    ...
    contextMenu.create({
      ...
      items: this.otherDrives.slice(0, 10).map(drive => ({
        ...
        '-',
        {
          ...
          label: 'Browse...',
          click: async () => {
            this.base = await broxme.shell.selectDriveDialog()
            this.load()
          }
        }
      ])
    ...
    contextMenu.create({
      ...
      items: this.otherDrives.slice(0, 10).map(drive => ({
        ...
        '-',
        {
          ...
          label: 'Browse...',
          click: async () => {
            this.target = await broxme.shell.selectDriveDialog()
            this.load()
          }
        }
      ])
...
class CompareDiffItemContent extends LitElement {
  ...
  renderLeftColumn () {
    if (this.diff.change === 'del' || this.diff.change === 'mod') {
      return this.renderFileContent(broxme.hyperdrive.drive(this.targetOrigin), this.diff.targetPath, this.diff.targetMountKey)
    }
    return ''
  }
  ...
  renderRightColumn () {
    if (this.diff.change === 'add' || this.diff.change === 'mod') {
      return this.renderFileContent(broxme.hyperdrive.drive(this.baseOrigin), this.diff.basePath, this.diff.baseMountKey)
    }
    return ''
  }
  ...
  async updated () {
    ...
    var [baseContent, targetContent] = await Promise.all([
      this.diff.change === 'del' || this.diff.change === 'mod' ? (broxme.hyperdrive.drive(this.targetOrigin)).readFile(this.diff.targetPath).catch(e => '') : '',
      this.diff.change === 'add' || this.diff.change === 'mod' ? (broxme.hyperdrive.drive(this.baseOrigin)).readFile(this.diff.basePath).catch(e => '') : '',
    ])
