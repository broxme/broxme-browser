...
import _debounce from 'broxme://app-stdlib/vendor/lodash.debounce.js'
import moment from 'broxme://app-stdlib/vendor/moment.js'
...
export class HistoryApp extends LitElement {
  ...
  constructor () {
    ...
    broxme.panes.attachToLastActivePane()
  ...
  async loadVisits (offset) {
    ...
    var rows = await broxme.history.getVisitHistory({
      before: +before,
      after: +after,
      offset,
      limit: BATCH_SIZE,
  ...
  onClickDelete (e, i) {
    ...
    broxme.history.removeVisit(v.url)
    this.requestUpdate()
  }
  ...
  onClickDeleteBulk () {
    ...
    if (period === 'all') {
      this.visits = []
      broxme.history.removeAllVisits()
      this.requestUpdate()
    } else {
      ...
      broxme.history.removeVisitsAfter(threshold)
