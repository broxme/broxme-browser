...
class GeneralSettingsView extends LitElement {
  ...
  async load () {
    // wire up events
    this.browserEvents = broxme.browser.createEventsStream()
    ...
    // fetch data
    this.browserInfo = await broxme.browser.getInfo()
    this.settings = await broxme.browser.getSettings()
    this.defaultProtocolSettings = await broxme.browser.getDefaultProtocolSettings()
    console.log('loaded', {
  ...
  render () {
   ...
   //<div class="form-group">    //This will be in upper section
    //<h2>Broxme Browser Analytics</h2>
    ...
    if (this.browserInfo && !this.browserInfo.isDaemonActive) {
      return html`
        <div class="section warning">
          ...
          <p>
            The "daemon" runs Broxme Browser's Hyperdrive networking.
          </p>
          <p>
      ...
      return html`
        <div class="section">
          <p class="message info">
            Sorry! Broxme Browser auto-updates are only supported on the production build for macOS and Windows.
          </p>

          <p>
            To get the most recent version of Broxme Browser, you'll need to <a href="https://github.com/broxme/broxme-browser">
            build Broxme Browser from source</a>.
          </p>
        </div>
      `
    }
          ...
          <div class="auto-updater">
            <p>
              ...
              <span class="up-to-date">
                ...
                Broxme Browser v${this.browserInfo.version} is up-to-date
              </span>
            </p>
          ...
          <div class="auto-updater">
            <p>
              ...
              <span class="version-info">
                ...
                Downloading the latest version of Broxme Browser...
              </span>
            </p>
            <p>
              ${this.renderAutoUpdateCheckbox()}
          ...
          <div class="auto-updater">
            <p>
              ...
              <span class="version-info">
                ...
                <strong>New version available.</strong> Restart Broxme Browser to install.
              </span>
            </p>
            <p>
              ${this.renderAutoUpdateCheckbox()}
  ...
  renderOnStartupSettings () {
    return html`
      <div class="section">
        <p>When Broxme Browser starts</p>
  ...
  renderRunBackgroundSettings () {
    return html`
      <div class="section on-startup">
        <p>
          Running in the background helps keep your data online even if you're not using Broxme Browser.
        </p>
      ...
      <div class="section">
        ...
        <div>
          <input name="new-tab"
                 id="newTab"
                 type="text"
                 value=${this.settings.new_tab || 'broxme://desktop/'}
  ...
  renderProtocolSettings () {
    const toggleRegistered = (protocol) => {
      ...
      if (this.defaultProtocolSettings[protocol]) {
        broxme.browser.setAsDefaultProtocolClient(protocol)
      } else {
        broxme.browser.removeAsDefaultProtocolClient(protocol)
      }
    ...
    return html`
      <div class="section">
        <p>Set Broxme Browser as the default browser for:</p>
  ...
  //     broxme.browser.setSetting('analytics_enabled', this.settings.analytics_enabled)
  //     this.requestUpdate()
  //     toast.create('Setting updated')
  //   }
  ...
  //           <li>Your Broxme Browser version, e.g. ${this.browserInfo.version}</li>
  ...
  onClickCheckUpdates () {
    // trigger check
    broxme.browser.checkForUpdates()
  }

  onClickCheckPrereleases (e) {
    e.preventDefault()
    broxme.browser.checkForUpdates({prerelease: true})
  }

  onClickRestart () {
    broxme.browser.restartBrowser()
  }

  onToggleAutoUpdate () {
    this.settings.auto_update_enabled = this.isAutoUpdateEnabled ? 0 : 1
    this.requestUpdate()
    broxme.browser.setSetting('auto_update_enabled', this.settings.auto_update_enabled)
    toast.create('Setting updated')
  }

  onCustomStartPageChange (e) {
    this.settings.custom_start_page = e.target.value
    broxme.browser.setSetting('custom_start_page', this.settings.custom_start_page)
    toast.create('Setting updated')
  }

  onBrowserThemeChange (e) {
    this.settings.browser_theme = e.target.value
    broxme.browser.setSetting('browser_theme', this.settings.browser_theme)
    toast.create('Setting updated')
  }

  onNewTabsInForegroundToggle (e) {
    this.settings.new_tabs_in_foreground = this.settings.new_tabs_in_foreground == 1 ? 0 : 1
    broxme.browser.setSetting('new_tabs_in_foreground', this.settings.new_tabs_in_foreground)
    toast.create('Setting updated')
  }

  onRunBackgroundToggle (e) {
    this.settings.run_background = this.settings.run_background == 1 ? 0 : 1
    broxme.browser.setSetting('run_background', this.settings.run_background)
    toast.create('Setting updated')
  }

  onNewTabChange (e) {
    this.settings.new_tab = e.target.value
    broxme.browser.setSetting('new_tab', this.settings.new_tab)
    toast.create('Setting updated')
  }

  async onClickBrowseNewTab (e) {
    var sel = await broxme.shell.selectFileDialog({
      allowMultiple: false
    })
    if (sel) {
  ...
  onClickDefaultNewTab (e) {
    this.settings.new_tab = 'broxme://desktop/'
    broxme.browser.setSetting('new_tab', this.settings.new_tab)
    toast.create('Setting updated')
    this.requestUpdate()
  }
  ...
  onSearchEngineChange (e) {
    ...
    broxme.browser.setSetting('search_engines', this.settings.search_engines)
    toast.create('Setting updated')
    this.requestUpdate()
  }
  ...
  onChangeDefaultZoom (e) {
    ...
    broxme.browser.setSetting('default_zoom', this.settings.default_zoom)
    toast.create('Setting updated')
  }
  ...
  async onClickRestartDaemon (e) {
    ...
    await broxme.browser.reconnectHyperdriveDaemon()
    this.browserInfo = await broxme.browser.getInfo()
    this.requestUpdate()
  }
  ...
  async onClickAddSiteToBeakerNetwork (e) {
    ...
    await broxme.browser.addProfileToBeakerNetwork()
