...
import { classMap } from 'broxme://app-stdlib/vendor/lit-element/lit-html/directives/class-map.js'
...
const EXPLORER_URL = drive => `broxme://explorer/${drive.url.slice('hyper://'.length)}`
...
export class DrivesView extends LitElement {
  ...
  async load () {
    var drives = await broxme.drives.list({includeSystem: false})
  ...
  async forkDrive (drive) {
    var drive = await broxme.hyperdrive.forkDrive(drive.url)
  ...
  async diffDrive (drive) {
    window.open(`broxme://diff/?base=${drive.url}`)
  }

  async driveProps (drive) {
    await broxme.shell.drivePropertiesDialog(drive.url)
    this.load()
  }

  async removeDrive (drive) {
    await broxme.drives.remove(drive.url)
    const undo = async () => {
      await broxme.drives.configure(drive.url)
