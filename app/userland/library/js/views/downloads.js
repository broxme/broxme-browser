...
import { toNiceDomain } from 'broxme://app-stdlib/js/strings.js'
...
export class DownloadsView extends LitElement {
  ...
  async load () {
    this.downloads = await broxme.downloads.getDownloads()
    console.log(this.downloads)

    var dlEvents = broxme.downloads.createEventsStream()
  ...
  onPauseDownload (download) {
    broxme.downloads.pause(download.id)
  }

  onResumeDownload (download) {
    broxme.downloads.resume(download.id)
  }

  onCancelDownload (download) {
    broxme.downloads.cancel(download.id)
  }
  ...
  onShowDownload (download) {
    broxme.downloads.showInFolder(download.id)
      .catch(err => {
  ...
  onOpenDownload (download) {
    broxme.downloads.open(download.id)
      .catch(err => {
        download.fileNotFound = true
        this.requestUpdate()
      })
  ...
  onRemoveDownload (download) {
    broxme.downloads.remove(download.id)
