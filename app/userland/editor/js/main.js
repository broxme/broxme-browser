...
class EditorApp extends LitElement {
  ...
  get drive () {
    return broxme.hyperdrive.drive(this.url)
  }
  ...
  ensureEditorEl () {
    if (!this.editorEl) {
      ...
      this.editorEl.addEventListener('contextmenu', async e => {
        var choice = await broxme.browser.showContextMenu([
          {id: 'cut', label: 'Cut'},
          {id: 'copy', label: 'Copy'},
  ...
  async createEditor () {
    ...
    return new Promise((resolve, reject) => {
      window.require.config({ baseUrl: 'broxme://assets/' })
    ...
    try {
      ...
      try {
        if (url.startsWith('hyper:')) {
          ...
          body = await broxme.browser.fetchBody(url)
        } else {
          this.isFilesOpen = false
      ...
      if (!this.dne && !this.isBinary) {
        ...
        if (body && model.getModeId() === 'plaintext') {
          let type = await broxme.browser.getResourceContentType(url)
          if (type) {
  ...
  async loadDrive (url) {
    ...
    // load drive meta
    let drive = broxme.hyperdrive.drive(url)
    ...
    this.mountInfo = undefined
    {
      ...
      while (pathParts.length) {
        ...
        if (stat && stat.mount) {
          this.mountInfo = await broxme.hyperdrive.drive(stat.mount.key).getInfo()
  ...
  async showMenu (x, y, folderPath, item, folderItemUrls) {
    ...
    if (item) {
      items.push({
        ...
        click () {
          broxme.browser.openUrl(item.url)
        }
      })
      items.push({
      ...
      items.push({
        ...
        click: () => {
          broxme.browser.newPane(item.shareUrl, {splitDir: 'vert'})
        }
      })
      items.push({
        label: 'Open in Pane Below',
        click: () => {
          broxme.browser.newPane(item.shareUrl, {splitDir: 'horz'})
        }
      })
      items.push({type: 'separator'})
      items.push({
  ...
  async onClickActions (e) {
    ...
    await contextMenu.create({
      ...
      y: rect.bottom,
      center: true,
      fontAwesomeCSSUrl: 'broxme://assets/font-awesome.css',
      noBorders: true,
      roomy: true,
      rounded: true,
      style: 'padding: 4px 0',
  ...
  onClickEditReal (e) {
    broxme.browser.openUrl(`broxme://editor?url=${this.mountInfo.url + this.mountInfo.resolvedPath}`, {
      setActive: true,
      adjacentActive: true
    })
  }
  ...
  async onClickView () {
    this.attachedPane = broxme.panes.getAttachedPane()
    if (!this.attachedPane) {
      this.attachedPane = await broxme.panes.create(this.url, {attach: true})
    } else {
      broxme.panes.navigate(this.attachedPane.id, this.url)
    }
  }
  ...
  async onClickSave () {
    ...
    if (this.attachedPane) {
      this.attachedPane = broxme.panes.getAttachedPane()
      broxme.panes.navigate(this.attachedPane.id, this.attachedPane.url)
    }
    this.setSaveBtnState()
    this.setFocus()
  }
    ...
    if (this.resolvedPath === oldpath) {
      ...
      if (this.attachedPane) {
        ...
        if (this.attachedPane.url === oldurl) {
          broxme.panes.navigate(this.attachedPane.id, urlp.toString())
        }
      }
    }
  }
  ...
  async onClickNewMount (folderPath) {
    if (this.readOnly) return
    var url = await broxme.shell.selectDriveDialog()
    if (!url) return
  ...
  async onClickImportFiles (folderPath) {
    ...
    try {
      var {numImported} = await broxme.shell.importFilesDialog(joinPath(this.drive.url, folderPath))
  ...
  async onClickImportFolders (folderPath) {
    toast.create('Importing...')
    try {
      var {numImported} = await broxme.shell.importFoldersDialog(joinPath(this.drive.url, folderPath))
      if (numImported > 0) toast.create('Import complete', 'success')
      else toast.destroy()
    } catch (e) {
      console.log(e)
  ...
  async onClickFork (e) {
    ...
    var newDrive = await broxme.hyperdrive.forkDrive(this.url)
    var newDriveUrlp = new URL(newDrive.url)
    urlp.hostname = newDriveUrlp.hostname
    
    broxme.browser.gotoUrl(urlp.toString())
    this.load(urlp.toString())
  }
}
