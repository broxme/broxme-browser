import { LitElement, html, TemplateResult } from 'broxme://app-stdlib/vendor/lit-element/lit-element.js'
...
import { joinPath, shortenAllKeys } from 'broxme://app-stdlib/js/strings.js'
import { findParent } from 'broxme://app-stdlib/js/dom.js'
...
class WebTerm extends LitElement {
  ...
  constructor () {
    ...
    this.addEventListener('click', e => {
      ...
      if (anchor) {
        ...
        if (e.metaKey || anchor.getAttribute('target') === '_blank') {
          ...
          var attachedPane = broxme.panes.getAttachedPane()
          if (attachedPane) {
            broxme.panes.navigate(attachedPane.id, anchor.getAttribute('href'))
          } else {
    ...
    ;(async () => {
      ...
      if (ctx && ctx.startsWith('broxme://webterm')) ctx = undefined
      var attachedPane = await broxme.panes.attachToLastActivePane()
      if (attachedPane) {
  ...
  async loadInstalledCommands () {
    var installed = await broxme.hyperdrive.readFile('hyper://private/webterm/installed.json').then(JSON.parse).catch(e => ([]))
    ...
    await Promise.all(installedValidated.map(async app => {
      let subcommands = await broxme.browser.spawnAndExecuteJs(app.url, `
        ;(() => {
          let commands = []
          if (broxme.terminal.getCommands().length) {
            for (let command of broxme.terminal.getCommands()) {
              commands.push({
                package: undefined,
                name: command.name,
                path: [command.name],
      ...
      var appCommand = {
        ...
        subcommands: subcommandsMap(
          app.name,
          name => (...args) => broxme.browser.spawnAndExecuteJs(app.url, `
            ;(() => {
              let command = broxme.terminal.getCommands().find(c => c.name === ${JSON.stringify(name)});
              if (command) {
  ...
  async loadPageCommands () {
    ...
    this.pageCommands = await broxme.panes.executeJavaScript(attachedPane.id, `
      ;(() => {
        let commands = {}
        if (broxme.terminal.getCommands().length) {
          for (let command of broxme.terminal.getCommands()) {
  ...
  async loadEnvVars () {
    try {
      this.envVars = await broxme.hyperdrive.readFile('hyper://private/webterm/env.json', 'json')
    } catch (err) {
  ...
  async persistEnvVars () {
    ...
    delete envVars['@']
    delete envVars.cwd
    await broxme.hyperdrive.writeFile('hyper://private/webterm/env.json', envVars, 'json')
  }
  ...
  async evalPrompt () {
    ...
    if (commandName.startsWith('@')) {
      ...
      if (command) {
        command.fn = (...args) => broxme.panes.executeJavaScript(attachedPane.id, `
          ;(() => {
            let command = broxme.terminal.getCommands().find(c => c.name === ${JSON.stringify(commandName.slice(1))});
            if (command) {
              return command.handle.apply(command, ${JSON.stringify(args)})
            }
          })();
        ...
        page: {
          goto (url, opts = {}) {
            if (opts.newTab) window.open(url)
              ...
              var pane = broxme.panes.getAttachedPane()
              if (!pane) throw new Error('No attached pane')
              broxme.panes.navigate(pane.id, url)
            }
          },
          refresh () {
            var pane = broxme.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            broxme.panes.navigate(pane.id, pane.url)
          },
          focus () {
            var pane = broxme.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            broxme.panes.focus(pane.id)
          },
          exec (js) {
            var pane = broxme.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            return broxme.panes.executeJavaScript(pane.id, js)
          },
          inject (css) {
            var pane = broxme.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            return broxme.panes.injectCss(pane.id, css)
          },
          uninject (id) {
            var pane = broxme.panes.getAttachedPane()
            if (!pane) throw new Error('No attached pane')
            return broxme.panes.uninjectCss(pane.id, id)
          }
        },
        out: (...args) => {
          args = args.map(arg => {
