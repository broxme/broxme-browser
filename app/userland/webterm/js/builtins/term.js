import { html } from 'broxme://app-stdlib/vendor/lit-element/lit-element.js'
...
export async function install (opts = {}, name, url) {
  ...
  if (url.startsWith('hyper://')) {
    let info = await broxme.hyperdrive.getInfo(url)
...
async function readInstalled () {
  return broxme.hyperdrive.drive('hyper://private/').readFile('/webterm/installed.json').then(JSON.parse).catch(e => ([]))
}

async function saveInstalled (urls) {
  await broxme.hyperdrive.drive('hyper://private/').mkdir('/webterm').catch(e => undefined)
  await broxme.hyperdrive.drive('hyper://private/').writeFile('/webterm/installed.json', JSON.stringify(urls, null, 2))
}
