...
export async function cd (opts = {}, location = '') {
  ...
  if (cwd.startsWith('hyper://')) {
    ...
    let drive = broxme.hyperdrive.drive(urlp.origin)
    let st
...
export async function edit (opts = {}, location = '') {
  ...
  await this.page.goto(`broxme://editor/?url=${location}`)
}
...
export const system = {
  async fs_audit_stream (opts = {}) {
    ...
    var stream = await broxme.logger.streamAuditLog()
