...
import { toHex, isSameOrigin, toNiceDomain, shorten } from 'broxme://app-stdlib/js/strings.js'
import { timeDifference } from 'broxme://app-stdlib/js/time.js'
import bytes from 'broxme://app-stdlib/vendor/bytes/index.js'
import 'broxme://app-stdlib/js/com/hover-card.js'
...
class HypercoreToolsApp extends LitElement {
  ...
  constructor () {
    ...
    broxme.panes.addEventListener('pane-attached', e => {
      if (!ignoreNextAttachEvent) {
        this.load(broxme.panes.getAttachedPane().url)
      }
  ...
  async load (url) {
    ...
    this.url = url
    this.drivecores = await broxme.hyperdebug.listCores(url)
    ...
    for (let drive of this.drivecores) {
      ...
      drive.metadata.downloadedBlockBits = await broxme.hyperdebug.hasCoreBlocks(drive.metadata.key, 0, drive.metadata.totalBlocks)
      drive.metadata.log = []
      this.bindCoreLogEvents(drive.metadata, drive.url, 'metadata')
      this.requestUpdate()

      drive.content.downloadedBlockBits = await broxme.hyperdebug.hasCoreBlocks(drive.content.key, 0, drive.content.totalBlocks)
      drive.content.log = []
      this.bindCoreLogEvents(drive.content, drive.url, 'content')
      this.requestUpdate()

      try {
        drive.files = await broxme.hyperdrive.readdir(drive.url, {recursive: true, includeStats: true})
  ...
  async bindApiLogEvents () {
    ...
    this.auditLogStream = await broxme.logger.streamAuditLog({caller: this.url, includeResponse: true})
  ...
  bindCoreLogEvents (core, url, corename) {
    ...
    var events = broxme.hyperdebug.createCoreEventStream(url, corename)
