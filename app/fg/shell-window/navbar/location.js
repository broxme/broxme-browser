...
class NavbarLocation extends LitElement {
  ...
  get isBeaker () {
    return this.url.startsWith('broxme://')
  }
  ...
  get modifiedUrl () {
    var url = this.url
    if (url.startsWith('broxme://desktop')) {
      url = ''
  ...
  render () {
    return html`
      ...
      <shell-window-navbar-site-info
        ...
        ?rounded=${false/*this.url.startsWith('broxme://desktop')*/}
    ...
    return html`
      <div class="input-container" @contextmenu=${this.onContextMenuLocation}>
        <input
          ...
          placeholder="Search or enter web address here"
  ...
  renderInputPretty () {
    if (this.url.startsWith('broxme://desktop')) {
      return html`
    ...
    if (/^(hyper|http|https|broxme|dat):\/\//.test(this.url)) {
      try {
        ...
        var cls = 'protocol'
        // if (['broxme:'].includes(protocol)) cls += ' protocol-trusted'
