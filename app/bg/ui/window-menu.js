    ...
    submenu: [
      {
        ...
        accelerator: 'Cmd+,',
        click (item) {
          if (win) tabManager.create(win, 'broxme://settings', {setActive: true})
          else createShellWindow({ pages: ['broxme://settings'] })
        }
      },
      { type: 'separator' },
      { label: 'Services', role: 'services', submenu: [] },
      { type: 'separator' },
      { label: 'Hide Broxme', accelerator: 'Cmd+H', role: 'hide' },
  ...
  var historyMenu = {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: function (item) {
          if (win) tabManager.create(win, 'broxme://history', {setActive: true})
          else createShellWindow({ pages: ['broxme://history'] })
        }
      },
      { type: 'separator' },
      {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: async function (item) {
          if (tab) tab.togglePaneByOrigin({url: 'broxme://editor/'})
        }
      },
      {
        id: 'toggleTerminal',
        ...
        accelerator: 'Ctrl+`',
        click: function (item) {
          if (tab) tab.togglePaneByOrigin({url: 'broxme://webterm/'})
        }
      },
      {
        ...
        enabled: !noWindows,
        click: async function (item) {
          if (tab) tab.togglePaneByOrigin({url: 'broxme://hypercore-tools/'})
        }
      },
      {
    ...
    submenu: [
      {
        ...
        accelerator: 'F1',
        click: function (item) {
          if (win) tabManager.create(win, 'https://www.broxme.com/', {setActive: true})
        }
      },
      {type: 'separator'},
      {
        ...
        label: 'Report Issue',
        click: function (item) {
          if (win) tabManager.create(win, 'https://github.com/broxme/broxme-browser/issues', {setActive: true})
        }
      },
      {
   ...
   submenu: [
     {
       ...
       click: async function (item) {
         if (tab) tab.togglePaneByOrigin({url: 'broxme://explorer/'})
       }
     },
     {type: 'separator'},
     {
     ...
     },
     {
       ...
       enabled: !!isDriveSite,
       async click (item) {
         if (win) tabManager.create(win, `broxme://diff/?base=${url}`, {setActive: true})
       }
     },
     { type: 'separator' },
     {
