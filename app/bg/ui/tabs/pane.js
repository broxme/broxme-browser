...
export class Pane extends EventEmitter {
  ...
  get siteTitle () {
    try {
      ...
      if (urlp.protocol === 'broxme:') {
        if (urlp.hostname === 'diff') return 'broxme '
        if (urlp.hostname === 'explorer') return 'broxme'
        if (urlp.hostname === 'history') return 'broxme'
        if (urlp.hostname === 'library') return 'broxme'
        if (urlp.hostname === 'settings') return 'broxme'
        if (urlp.hostname === 'webterm') return 'broxme'
        return 'broxme'
      }
  ...
  get siteTrust () {
    try {
      ...
      if (['https:', 'broxme:'].includes(urlp.protocol)) {
        return 'trusted'
      }
  ...
  destroy () {
    if (this.url && !this.url.startsWith('broxme://')) {
