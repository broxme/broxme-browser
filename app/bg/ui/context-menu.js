...
export default function registerContextMenu () {
  ...
  app.on('web-contents-created', (e, webContents) => {
    webContents.on('context-menu', async (e, props) => {
      ...
      if (props.pageURL == 'broxme://shell-window/') { return }
      if (props.pageURL.startsWith('broxme://modals')) {
      ...
      if (isHyperdrive) {
        menuItems.push({
          ...
          click: async (item, win) => {
            if (targetTab) targetTab.createOrFocusPaneByOrigin({url: 'broxme://editor/', setActive: true})
          }
        })
        menuItems.push({
          label: 'Explore Files',
          click: async (item, win) => {
            if (targetTab) targetTab.createOrFocusPaneByOrigin({url: 'broxme://explorer/', setActive: true})
          }
        })
      }
      menuItems.push({ type: 'separator' })
