...
export function createShellWindow (windowState, createOpts = {dontInitPages: false}) {
  ...
  win.loadURL('broxme://shell-window')
...
export function getActiveWindow () {
  ...
  if (!win || win.webContents.getURL() !== 'broxme://shell-window/') {
    win = BrowserWindow.getAllWindows().filter(win => win.webContents.getURL() === 'broxme://shell-window/').pop()
  }
  return win
}
...
function userWantsToRestoreSession () {
  let answer = dialog.showMessageBoxSync({
    type: 'question',
    message: 'Sorry! It looks like Broxme Browser did not exit properly',
