  ...
  const respondBuiltinFrontend = async () => {
    return respond({
      ...
      headers: {
        ...
        'Content-Security-Policy': `default-src broxme:; img-src * data: asset: blob:; media-src * data: asset: blob:; style-src broxme: 'unsafe-inline';`,
...
<html>
  <head>
    ...
    <script type="module" src="broxme://drive-view/index.js"></script>
  </head>
</html>`)
    })
  }
  ...
  const respondError = (code, status, errorPageInfo) => {
    ...
    if (accept.includes('text/html')) {
      respond({
        ...
        headers: {
          'Content-Type': 'text/html',
          'Content-Security-Policy': "default-src 'unsafe-inline' broxme:;",
          'Access-Control-Allow-Origin': corsHeader,
          'Allow-CSP-From': '*'
        },
    ...
    if (!entry) {
      ...
      if (res?.err && /(not available|connectable)/i.test(res?.err.toString())) {
        return respondError(404, 'File Not Available', {
          ...
          errorInfo: `Broxme could not find any peers to access ${urlp.path}`,
          title: 'File Not Available'
        })
      }
      return respondError(404, 'File Not Found', {
        errorDescription: 'File Not Found',
        errorInfo: `Broxme could not find the file at ${urlp.path}`,
