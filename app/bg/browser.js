...
const DOT_DESKTOP_FILENAME = 'appimagekit-broxme-browser.desktop'
...
export async function setup () {
  ...
  session.defaultSession.webRequest.onBeforeRequest((details, cb) => {
    if (details.url.startsWith('asset:') || details.url.startsWith('broxme:')) {
      ...
      if (/^(broxme:\/\/|hyper:\/\/private\/)/.test(wc.getURL())) {
...
export function restartBrowser () {
  if (updaterState == UPDATER_STATUS_DOWNLOADED) {
    ...
    logger.info('Restarting Broxme by restartBrowser()')
    // do a simple restart
    app.relaunch()
