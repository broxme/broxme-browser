...
gulp.task('postbuild', gulp.series(function () {
  ...
  names.forEach(function (name) {
    // windows assets:
    if (name.indexOf('Broxme Browser Setup') === 0 && name.indexOf('.exe') !== -1) {
      var newName = 'broxme-browser-setup-' + name.slice('Broxme Browser Setup '.length)
      return cwd.move(name, newName)
    }

    // osx assets:
    if (name.indexOf('Broxme Browser') === 0 && (name.indexOf('.dmg') !== -1 || name.indexOf('-mac.zip') !== -1)) {
      var newName = 'broxme-browser' + name.slice('Broxme Browser'.length)
      return cwd.move(name, newName)
    }
  })
